...
def Message processData(Message message) {
    ...
    def email_body = ''
    def headers_text = ''
    def response = message.getProperty("http.ResponseBody")
    def sap_msg = headers.get("sap-message")
    if (response != null) {
        ...
        //log S4 response header variable 'sap-message' 
        if(sap_msg != '' && sap_msg != null ){
            def messageLog5 = messageLogFactory.getMessageLog(message)
            messageLog5.addAttachmentAsString('Response Message', sap_msg, 'text/plain')
        }
        // log the S4 error response as an attachment
        if (add_to_log) {
               // save the http error response as a log attachment
                def messageLog = messageLogFactory.getMessageLog(message)
                messageLog.addAttachmentAsString('ResponseBody', response, 'text/plain')
        }
        if (add_to_log) {
            // add headers
            headers.each{ k, v -> 
