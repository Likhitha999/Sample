...
def Message processData(Message message) {
    ...
    def lockedwarehouse = map.get("lockedwarehouse") ?: 'N'
    def email_body = ''
    def headers_text = ''
    ...
    if (ex != null) {
        ...
        if (ex.getClass().getCanonicalName().equals('org.apache.camel.component.ahc.AhcOperationFailedException')) {
            ...
            if (add_to_log){
                def exceptionmsg
                if (lockedwarehouse == 'Y'){
                    exceptionmsg = "Failed to load the warehouse due to a lock issue" + "\n" + "\n" + map.get('exception')
                }
                messageLog5.addAttachmentAsString('ResponseBody', exceptionmsg, 'text/plain')
            }
        }
        if (add_to_log) {
            // add headers
            ...
            // save request payload as a log attachment
